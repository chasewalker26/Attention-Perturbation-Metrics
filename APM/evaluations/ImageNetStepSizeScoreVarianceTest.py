...
def run_and_save_tests(img_hw, transform, image_count, batch_size, model, explainer, model_name, test_type, device, dataset_path, dataset_name, num_patches):
    ...
    base_step_size = img_hw
    step_modifiers = [0.25, 0.5, 0.75, 1, 2, 3, 4]
    ...
    for image in sorted(os.listdir(dataset_path)):    
        ...
        print(model_name + " " + test_type + " step size test, image: " + image + " " + str(images_used + 1) + "/" + str(image_count))
        ...
        score_list_a = np.zeros((len(step_modifiers), num_attrs))
        score_list_b = np.zeros((len(step_modifiers), num_attrs))

        # capture scores for the attributions
        for i in range(len(step_modifiers)):
            step_size = int(base_step_size * step_modifiers[i])
            ...
            if test_type == "RISE":
                ...
                test_b = Mono.MonotonicityMetric(model, img_hw * img_hw, 'positive', step_size, substrate_fn = torch.zeros_like)
    ...
    file_name = dataset_name + "_" + test_type + "_step_size_test_" + str(image_count) + "_images"
