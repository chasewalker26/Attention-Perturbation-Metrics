...
class Baselines:
    ...
    def generate_raw_attn(self, input, device):
    ...
    def generate_grad(self, input, target_class, device):
        ...
        grad = grad[0, :, 0, 1:].reshape(-1, patches_per_side, patches_per_side).mean(0, keepdim = True)

        return grad

    # https://github.com/hila-chefer/Transformer-Explainability/blob/main/baselines/ViT/ViT_explanation_generator.py
    def generate_cam_attn(self, input, target_class, device):
        input.requires_grad = True
        output = self.model(input.to(device), register_hook=True)
        score = output[0][target_class].sum()
        score.backward()
    ...
    def bidirectional(self, input, target_class, steps=20, start_layer=4, samples=20, noise=0.2, mae=False, dino=False, ssl=False, device = "cuda"):
        input.requires_grad = True
        output = self.model(input.to(device), register_hook=True)
        score = output[0][target_class].sum()
        score.backward()
