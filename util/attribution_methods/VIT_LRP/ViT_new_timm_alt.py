...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.attention_map_pre_mask = None
    ...
    def save_attention_map_pre_mask(self, attention_map_pre_mask):
        self.attention_map_pre_mask = attention_map_pre_mask

    def get_attention_map_pre_mask(self):
        return self.attention_map_pre_mask
    ...
    def save_qkv_res(self, qkv_res):
        self.qkv_res = qkv_res

    def get_qkv_res(self):
        return self.qkv_res
    ...
    def forward(self, x, register_hook=False):
        ...
        self.save_attention_map_pre_mask(attn)
...
class Block(nn.Module):
    ...
    def get_block_out(self):
        return self.block_out
    ...
    def save_block_out(self, block_out):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.patch_embedding_grads = None
    ...
    def save_block_inputs(self, block_inputs):
        self.block_inputs = block_inputs

    def get_block_inputs(self):
        return self.block_inputs

    def save_patch_embedding_grads(self, patch_embedding_grads):
        self.patch_embedding_grads = patch_embedding_grads
        return
    
    def get_patch_embedding_grads(self):
        return self.patch_embedding_grads

    def forward_features(self, x, attn_mask = None, register_hook=False):
        x = self.patch_embed(x) 

        if register_hook:
            x.register_hook(self.save_patch_embedding_grads)
        ...
        # completely remove tokens
        # attn_mask bx1xsxs 
        if attn_mask is not None:
            reduced_mask = attn_mask[0, 0, 0].clone()

            mask_locs = torch.where(reduced_mask != 0)[0]
            no_mask_locs = torch.where(reduced_mask == 0)[0]
            reduced_mask[mask_locs] = False
            reduced_mask[no_mask_locs] = True
            reduced_mask = reduced_mask.type(torch.bool)
            x = x[:, reduced_mask]
        ...
        self.save_block_inputs(block_inputs)
    ...
    def forward(self, x, attn_mask = None, register_hook=False):
        x = self.forward_features(x, attn_mask, register_hook=register_hook)
