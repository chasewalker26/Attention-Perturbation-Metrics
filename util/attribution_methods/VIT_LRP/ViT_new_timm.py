...
class Attention(nn.Module):
    ...
    def forward(self, x, attn_mask, register_hook=False):
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def forward(self, x, attn_mask, register_hook=False):
        ...
        x = x + self.attn(self.norm1(x), attn_mask = attn_mask, register_hook=register_hook)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, attn_mask, register_hook=False):
        ...
        for blk in self.blocks:
            x = blk(x, attn_mask = attn_mask, register_hook=register_hook)
