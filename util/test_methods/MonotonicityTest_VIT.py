...
class MonotonicityMetric():
    ...
    def single_run(self, img_tensor, saliency_map, patch_mask, device, max_batch_size = 50):
        ...
        if self.mode == "negative":
            monotonicity = spearmanr(np.linspace(n_steps + 1, 1, n_steps + 1), model_response).correlation
        elif self.mode == "positive":
            monotonicity = spearmanr(np.linspace(1, n_steps + 1, n_steps + 1), model_response).correlation
